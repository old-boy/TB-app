...
router.post('/update/:id',(req,res,next) => {
	...
	BuyerInfo.updateOne({ _id }, req.body, (err, data) => {
		if (err) {
...
router.post('/add',(req,res,next) => {
    ...
    BuyerInfo.findOne({buyerName:req.body.buyerName}).then((data)　=> {
        if(data){
            ...
            let buyerInfoEntity = new Faq(newBuyerInfo)
            buyerInfoEntity.save(err => {
                if (err) {
                    res.json({
                        status: '0',
                        msg: err.message,
...
router.delete('/del/:id',(req,res,next) => {
	...
	BuyerInfo.deleteOne({ _id: id }).then((data) => {
		// console.log(user)
		console.log(data)
		if(data){
			res.status(200).json({
